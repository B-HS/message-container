...
- [x] 행 갱신 재스캔·로그 (마이그레이션 0002) — 2026-08-25 확인: 3 provider 모두 실 DB 로 "기존 행 text 수정·읽음 처리(date_read) 재동기화 + tapback 행 associated 컬럼 동기화 + `/api/logs`(sync.batch·auth 이벤트)" 검증. sqlite 는 로컬 `bun index.ts`(34110), mysql 은 `mysql:8`(34306)·postgres 는 `postgres:18`(34432) 컨테이너에 로컬 서버(34120/34130) 연결. 재확인 방법: fake chat.db 에 `UPDATE message SET text=... WHERE ROWID=...` 후 `POST /api/sync/run` → `GET /api/chats/:id/messages` 로 반영 확인
